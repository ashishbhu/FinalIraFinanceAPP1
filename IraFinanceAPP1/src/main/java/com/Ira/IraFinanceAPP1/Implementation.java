...
/*14.------ok------------------DETAILS OF REPORT by Date-----------------------------------------------------------------*/		
		public String getReportHDR(String parent,String date1,String date2)
		{
			...
			JSONObject jo=new JSONObject();
				...
				String subuser1="'"+parent+"'";
				String regis="select  emailid from registration where  subid="+parent1;
				String subu="select  subId from subuser where childUserName="+subuser1;
				
				try
				{
					Statement st11=con.createStatement();
					ResultSet rs11=st11.executeQuery(regis);
					
					if(rs11.next()==false)   /* if user is not main user */
					{
						System.out.println("mainuser");
						
						Statement st12=con.createStatement();
						ResultSet rs12=st12.executeQuery(subu);
						
						if(rs12.next()==false)  /* if user is not sub user */
						{
							System.out.println("subuser1");
							jo.put("check", "unotexist");
				 			return jo.toString();
							
						}
						else         /* if user is sub user */
						{
							//rs12.previous();
							System.out.println(rs12.getInt(1));
							int parentid=rs12.getInt(1);
							String regis1="select emailid from registration where  subid="+parentid;
							Statement st14=con.createStatement();
							ResultSet rs14=st14.executeQuery(regis1);
							
							if(rs14.next()==false)     /* getting email id from parent of sub user*/
							{
								jo.put("check", "unotexist");
								return jo.toString();
							}
					...
					else   /* if user is parent user get email id*/
					{
						System.out.println("mainuser");
						//rs11.previous();
						tomail=rs11.getString(1);
					}
				} 
				catch (SQLException e1)
				{
					System.out.println(e1);
					logger.error("In Details of report Service: error: "+e1);
